...
def main():
	...
	out_filename = lambda class_count_, class_index_, coords_: os.path.join(out_path, f'{class_index_}_{"_".join(map(str,coords_))}_{class_count_}.jpg')
		...
		for bbox in find_region_proposals(image):
			...
			for classname, box in zip(classnames, boxes):
				...
				if iou > 0.7 and class_count[class_index]<LIMIT_CLASS_IMAGES:
					out_filename_ = out_filename(total_class_count[class_index],class_index,coords)
					cv2.imwrite(out_filename_, cv2.resize(image[y1:y2,x1:x2], image_size))
					class_count[class_index] += 1
					total_class_count[class_index] += 1
			
			if max_iou < 0.3 and class_count[0]<LIMIT_CLASS_IMAGES:
				cv2.imwrite(out_filename(total_class_count[0],0,(0,0,1000,1000)), cv2.resize(image[y1:y2,x1:x2], image_size))
				class_count[0] += 1
